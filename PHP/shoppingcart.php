       ...
       if ($result_customer->num_rows > 0) {
           ...
           if (isset($_GET['product_id'])) {
               ...
               $check_cart_result = $conn_product->query($check_cart_query);
                ...
                /*
               $sql_reduce_quantity ="UPDATE Shopping_cart set Quantity = '$quantity' - 1 WHERE Customer_ID = '$Customer_ID' AND product_id = '$product';";
               $result_reduce = $conn_product->query($sql_reduce_quantity); 
               if $quantity < 1
               {
                $sql_reduce_quantity = "DELETE FROM Shopping_cart WHERE Customer_ID = '$Customer_ID' AND product_id = '$product_id';"
                $result_reduce = $conn_product->query($sql_reduce_quantity);
               }

               $sql_increase_quantity =" UPDATE Shopping_cart set Quantity = '$quantity' + 1 WHERE Customer_ID = '$Customer_ID' AND product_id = '$product';";
               $result_increase = $conn_product->query($sql_increase_quantity); 
                */
               ...
               else 
               {
                ...
                if ($check_cart_result->num_rows == 0 && $result_select_insert->num_rows == 0 ) 
                {
            ...
            echo "<p class = 'shopcust'>Please login to add items to the cart</p>";
                ...
                $result_select = $conn_product->query($sql_select);
                                    ...
                                    foreach ($product_ids as $product_id) {
                                        // Fetch the quantity for this product from the shopping cart
                                        $quantity_query = "SELECT Quantity FROM Shopping_cart WHERE Customer_ID = '$Customer_ID' AND product_id = '$product_id'";
                                        $quantity_result = $conn_product->query($quantity_query);
                                        ...
                                        // Include a hidden input field for the quantity
                                        echo '<input type="hidden" name="quantity[]" value="' . $quantity . '">';
                                    }



                    require_once 'insertcart.php'; 
                    echo "<br>"; 
                    echo "<br>"; 
